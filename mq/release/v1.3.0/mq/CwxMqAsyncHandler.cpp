...
int CwxMqAsyncHandler::onRecvMsg(CwxMsgBlock*& msg, CwxAppTss* pThrEnv)
{
    ...
    if (CwxMqPoco::MSG_TYPE_SYNC_DATA_REPLY == msg->event().getMsgHeader().getMsgType())
    {
        ...
        if (!conn->m_window.recvOneMsg(ullSid))
        {
            ...
            conn->m_window.getNextRecvMsg(ullMinSid);
        ...
        ///发送下一条binlog
        iState = sendBinLog(m_pApp, conn, pTss, true);
        if (-1 == iState)
        {
        ...
        {///产生continue的消息
            noticeContinue(pTss, conn->m_window.getConnId());
        }
        ///返回
        return 1;
    }
    else if (CwxMqPoco::MSG_TYPE_SYNC_REPORT == msg->event().getMsgHeader().getMsgType())
    {
        do
        {
            ...
            iState = sendBinLog(m_pApp, conn, pTss, false);
            if (-1 == iState)
            {
                iRet = CWX_MQ_INNER_ERR;
                CWX_ERROR((pTss->m_szBuf2K));
...
int CwxMqAsyncHandler::onUserEvent(CwxMsgBlock*& msg, CwxAppTss* pThrEnv)
{
    ...
    {
        conn = iter->second;
        int iState = sendBinLog(m_pApp, conn, pTss, true);
        if (-1 == iState)
        {
...
void CwxMqAsyncHandler::noticeContinue(CwxMqTss* , CWX_UINT32 uiConnId)
{
