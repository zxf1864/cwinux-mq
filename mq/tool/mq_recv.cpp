...
int main(int argc, char** argv) {
  ...
  CWX_UINT32 num = 0;
  do {
    if (CWX_MQ_ERR_SUCCESS != CwxMqPoco::packReportData(&writer,
      block,
      0,
    ...
    while (1) {
      ...
      if (CwxMqPoco::MSG_TYPE_SYNC_REPORT_REPLY == head.getMsgType()) {
        if (CWX_MQ_ERR_SUCCESS != CwxMqPoco::parseReportDataReply(&reader,
          block,
          ullSessionId,
          szErr2K))
        {
        ...
        for (CWX_UINT16 i = 0; i < reader_chunk.getKeyNum() - bSign; i++) {
          ...
          num++;
          if (0 != output(reader, reader_chunk.getKey(i)->m_szData,
        ...
        if (CWX_MQ_ERR_SUCCESS != CwxMqPoco::parseSyncErr(&reader,
          block,
          iRet,
          pErrMsg,
          szErr2K))
