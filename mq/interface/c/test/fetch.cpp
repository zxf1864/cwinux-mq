...
int main(int argc, char** argv) {
  ...
  if (CWX_MQ_ERR_SUCCESS
      != cwx_mq_pack_fetch_mq(writer, szSendBuf, &uiSendLen, 1, argv[3],
          argv[4], argv[5], 0, szErr2K)) {
  ...
  while (1) {
    ...
    if (CWX_MQ_MSG_TYPE_FETCH_DATA_REPLY == head.getMsgType()) {
      if (CWX_MQ_ERR_SUCCESS
          != cwx_mq_parse_fetch_mq_reply(reader, msg->rd_ptr(), msg->length(),
              &ret, &szErrMsg, &ullSid, &uiTimestamp, &data, &group, &type,
              &attr, szErr2K)) {
