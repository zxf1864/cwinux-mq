...
int CwxBinLogCursor::open(char const* szFileName, CWX_UINT32 uiFileNo,
  ...
  if (-1 == m_fd) {
    CwxCommon::snprintf(this->m_szErr2K, 2047, "Can't open file:%s.",
        szFileName);
    return -1;
  }
...
int CwxBinLogCursor::data(char * szBuf, CWX_UINT32& uiBufLen) {
  ...
  if (-1 == m_fd) {
    CwxCommon::snprintf(this->m_szErr2K, 2047,
        "Cursor's file handle is invalid");
    return -1;
  }
  ...
  if (m_curLogHeader.getLogLen()) {
    if (uiBufLen < m_curLogHeader.getLogLen()) {
      CwxCommon::snprintf(this->m_szErr2K, 2047,
          "Buf is too small, buf-size[%u], data-size[%u]", uiBufLen,
          m_curLogHeader.getLogLen());
      return -1;
    }
    iRet = pread(this->m_fd, szBuf, m_curLogHeader.getLogLen(),
...
inline bool CwxBinLogCursor::preadPage(int fildes, CWX_UINT32 uiBlockNo,
  ...
  if ((uiBlockNo == m_uiBlockNo) && (uiOffset <= m_uiBlockDataOffset))
    return true;
  do {
  ...
  CwxCommon::snprintf(this->m_szErr2K, 2047,
      "Failure to read bin-log file:%s, errno=%d.", m_strFileName.c_str(),
      errno);
  return false;
}
...
ssize_t CwxBinLogCursor::pread(int fildes, void *buf, size_t nbyte,
  ...
  if (nbyte) {
    ...
    uiBlockEndOffset =
        uiBlockNum == 1 ?
            uiBlockStartOffset + nbyte : (CWX_UINT32) BINLOG_READ_BLOCK_SIZE;
    if (!preadPage(fildes, uiStartBlock, uiBlockEndOffset))
      return -1;
    if (uiBlockEndOffset <= m_uiBlockDataOffset) { //数据足够
      memcpy(buf, m_szReadBlock + uiBlockStartOffset,
          uiBlockEndOffset - uiBlockStartOffset);
      ...
      if (uiBlockStartOffset < m_uiBlockDataOffset) {
        memcpy(buf, m_szReadBlock + uiBlockStartOffset,
            m_uiBlockDataOffset - uiBlockStartOffset);
    ...
    //get last block
    {
      ...
      if (!preadPage(fildes, uiEndBlock, uiBlockEndOffset))
        return -1;
      if (uiBlockEndOffset <= m_uiBlockDataOffset) { //数据足够
...
int CwxBinLogIndexWriteCache::append(CwxBinLogHeader const& header,
  ...
  if (m_uiIndexLen + CwxBinLogIndex::BIN_LOG_INDEX_SIZE
      > CwxBinLogIndex::BIN_LOG_INDEX_SIZE
          * BINLOG_WRITE_INDEX_CACHE_RECORD_NUM) {
    if (0 != flushIndex(szErr2K))
      return -1;
  }
...
CwxBinLogMgr::CwxBinLogMgr(char const* szLogPath, char const* szFilePrex,
    ...
    CWX_UINT32 uiBinlogFlushSecond, bool bDelOutManageLogFile) {
  m_bValid = false;
...
int CwxBinLogMgr::prev(CwxBinLogCursor* pCursor) {
  ...
  //Now, iRet=0
  map<CWX_UINT32/*file no*/, CwxBinLogFile*>::iterator iter =
      m_binlogMap.lower_bound(pCursor->getFileNo());
  CWX_ASSERT(iter != m_binlogMap.end());
  if (iter->first == pCursor->getFileNo())
    iter--;
  if (iter != m_binlogMap.end()) {
...
int CwxBinLogMgr::prev(CwxBinLogCursor* pCursor, char* szData,
    CWX_UINT32& uiDataLen) {
...
CWX_INT64 CwxBinLogMgr::leftLogNum(CwxBinLogCursor const* pCursor) {
  ...
  if (pCursor->isReady())
    return -1;
  CWX_INT64 num = m_binlogMap.find(pCursor->getFileNo())->second->getLogNum()
      - pCursor->getHeader().getLogNo();
  map<CWX_UINT32/*file no*/, CwxBinLogFile*>::iterator iter =
      m_binlogMap.upper_bound(pCursor->getFileNo());
  while (iter != m_binlogMap.end()) {
