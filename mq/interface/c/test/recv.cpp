...
int main(int argc, char** argv) {
  ...
  if (CWX_MQ_ERR_SUCCESS
      != cwx_mq_pack_sync_report(writer, 0, buf, &len,
          strtoull(argv[5], NULL, 0), 0, 0, argv[6], argv[3], argv[4], "",
          false, buf)) {
  ...
  while (1) {
    ...
    printf("recv msg: msg_attr=%u, msg_type=%u, data_len=%u\n", head.getAttr(),
        head.getMsgType(), head.getDataLen());
    if (CWX_MQ_MSG_TYPE_SYNC_DATA == head.getMsgType()) {
      if (CWX_MQ_ERR_SUCCESS
          != cwx_mq_parse_sync_data(reader, msg->rd_ptr(), msg->length(),
              &ullSid, &uiTimestamp, &data, &group, &type, &attr, szErr2K)) {
      ...
      len = 4096;
      if (CWX_MQ_ERR_SUCCESS
          != cwx_mq_pack_sync_data_reply(writer, head.getTaskId(), buf, &len,
              ullSid, buf)) {
      ...
      if (CWX_MQ_ERR_SUCCESS
          != cwx_mq_parse_sync_report_reply(reader, msg->rd_ptr(),
              msg->length(), &ret, &ullSid, &szErrMsg, szErr2K)) {
