  ...
  while ((option = cmd_option.next()) != -1) {
    switch (option) {
      case 'h':
        ...
        printf(
            "-n: recieve message's number, default is 1.zero is all from the sid.\n");
      ...
      case 'c':
        ...
        if (g_chunk > CWX_MQ_MAX_CHUNK_KSIZE)
          g_chunk = CWX_MQ_MAX_CHUNK_KSIZE;
        break;
      case 'z':
        g_zip = true;
        break;
...
static int output(CwxPackageReader& reader, char const* szMsg,
    CWX_UINT32 uiMsgLen) {
  CWX_UINT64 ullSid = 0;
  ...
  if (CWX_MQ_ERR_SUCCESS
      != CwxMqPoco::parseSyncData(&reader, szMsg, uiMsgLen, ullSid, timestamp,
          item, szErr2K)) {
    printf("failure to unpack recieve msg, err=%s\n", szErr2K);
    return -1;
  }
  printf("%s|%u|%s\n", CwxCommon::toString(ullSid, szErr2K, 10), timestamp,
      item->m_szData);
  return 0;
}

static bool checkSign(char const* data, CWX_UINT32 uiDateLen,
...
int main(int argc, char** argv) {
  ...
  do {
    if (CWX_MQ_ERR_SUCCESS
        != CwxMqPoco::packReportData(&writer, block, 0,
            g_sid > 0 ? g_sid - 1 : g_sid, g_sid == 0 ? true : false, g_chunk,
            g_source.length() ? g_source.c_str() : NULL, g_user.c_str(),
    ...
    while (1) {
      ...
      if (CwxMqPoco::MSG_TYPE_SYNC_REPORT_REPLY == head.getMsgType()) {
        if (CWX_MQ_ERR_SUCCESS
            != CwxMqPoco::parseReportDataReply(&reader, block, ullSessionId,
                szErr2K)) {
        ...
        if (head.isAttr(CwxMsgHead::ATTR_COMPRESS)) {
          ...
          if (!CwxZlib::unzip(g_unzip, g_unzip_len,
        ...
        if (0
            != output(reader,
                head.isAttr(CwxMsgHead::ATTR_COMPRESS) ?
                    (char const*) g_unzip : block->rd_ptr() + sizeof(ullSeq),
                head.isAttr(CwxMsgHead::ATTR_COMPRESS) ?
                    g_unzip_len : block->length() - sizeof(ullSeq))) {
          iRet = 1;
          break;
        }
        iRet = 0;
        ...
        block = NULL;
        if (CWX_MQ_ERR_SUCCESS
            != CwxMqPoco::packSyncDataReply(&writer, block, 0,
                CwxMqPoco::MSG_TYPE_SYNC_DATA_REPLY, ullSeq, szErr2K)) {
        ...
        if (!reader_chunk.unpack(
            head.isAttr(CwxMsgHead::ATTR_COMPRESS) ?
                (char const*) g_unzip : block->rd_ptr() + sizeof(ullSeq),
            head.isAttr(CwxMsgHead::ATTR_COMPRESS) ?
                g_unzip_len : block->length() - sizeof(ullSeq), false, false)) {
        ...
        if (g_sign.length()) {
          ...
          if (pItem) { //存在签名key
            if (!checkSign(reader_chunk.getMsg(),
                pItem->m_szKey - CwxPackage::getKeyOffset()
                    - reader_chunk.getMsg() - sizeof(ullSeq), pItem->m_szData,
                g_sign.c_str())) {
        ...
        for (CWX_UINT16 i = 0; i < reader_chunk.getKeyNum() - bSign; i++) {
          ...
          num++;
          if (0
              != output(reader, reader_chunk.getKey(i)->m_szData,
        ...
        if (CWX_MQ_ERR_SUCCESS
            != CwxMqPoco::packSyncDataReply(&writer, block, 0,
                CwxMqPoco::MSG_TYPE_SYNC_DATA_CHUNK_REPLY, ullSeq, szErr2K)) {
          printf("failure to pack receive data reply package, err=%s\n",
              szErr2K);
          iRet = 1;
          break;
        ...
        if (CWX_MQ_ERR_SUCCESS
            != CwxMqPoco::parseSyncErr(&reader, block, iRet, pErrMsg,
                szErr2K)) {
