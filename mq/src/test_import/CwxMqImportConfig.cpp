...
int CwxMqImportConfig::loadConfig(string const & strConfFile){
    ...
    if ((NULL == (pValue=parser.getElementAttr("svr_def:data", "size"))) || !pValue[0]){
        snprintf(m_szError, 2047, "Must set [svr_def:data:size].");
        return -1;
    }
    m_unDataSize =strtoul(pValue, NULL, 0);
    // group
    if ((NULL == (pValue=parser.getElementAttr("svr_def:data", "group"))) || !pValue[0]){
        snprintf(m_szError, 2047, "Must set [svr_def:data:group].");
        return -1;
    }
    m_uiGroup =strtoul(pValue, NULL, 0);
    // data size
    if ((NULL == (pValue=parser.getElementAttr("svr_def:data", "type"))) || !pValue[0]){
        snprintf(m_szError, 2047, "Must set [svr_def:data:type].");
        return -1;
    }
    m_uiType =strtoul(pValue, NULL, 0);
    //load listen
