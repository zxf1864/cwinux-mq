...
class CwxMqQueue {
  public:
    CwxMqQueue(string strName, string strUser, string strPasswd,
  ...
  public:
    ...
    ///2：达到了搜索点，但没有发现消息；
    ///-1：失败；
    int getNextBinlog(CwxMqTss* pTss, CwxMsgBlock*&msg, int& err_num,
        char* szErr2K);
  ...
  private:
    ...
    ///2：达到了搜索点，但没有发现消息；
    ///-1：失败；
    int fetchNextBinlog(CwxMqTss* pTss, CwxMsgBlock*&msg, int& err_num,
        char* szErr2K);
  private:
...
class CwxMqQueueMgr {
  ...
  public:
    ...
    int endSendMsg(string const& strQueue, CWX_UINT64 ullSid, bool bSend = true,
        char* szErr2K = NULL);
    ...
    ///添加队列
    int addQueue(string const& strQueue, CWX_UINT64 ullSid,
        string const& strUser, string const& strPasswd, char* szErr2K = NULL);
    ///1：成功
    ///0：不存在
    ///-1：其他错误
    ...
    //-1：权限失败；0：队列不存在；1：成功
    inline int authQueue(string const& strQueue, string const& user,
        string const& passwd) {
      CwxReadLockGuard<CwxRwLock> lock(&m_lock);
