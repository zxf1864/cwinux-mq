#ifndef __CWX_MQ_MC_RECV_HANDLER_H__
#define __CWX_MQ_MC_RECV_HANDLER_H__
/*
版权声明：
...
class CwxMqMcRecvHandler: public CwxCmdOp
{
public:
    ///构造函数
    CwxMqMcRecvHandler(CwxMqApp* pApp):m_pApp(pApp)
    {
        m_uiUnSyncLogNum = 0; ///<上次形成sync记录以来的新记录数
        m_ttLastSyncTime = 0; ///<上一次形成sync记录的时间
    }
    ///析构函数
    virtual ~CwxMqMcRecvHandler()
    {
