...
int CwxMqMasterHandler::createSession(CwxMqTss* pTss) {
  ...
  CwxINetAddr addr;
  if (0
      != addr.set(m_pApp->getConfig().getMaster().m_master.getPort(),
          m_pApp->getConfig().getMaster().m_master.getHostName().c_str())) {
    CWX_ERROR(
        ("Failure to init addr, addr:%s, port:%u, err=%d", m_pApp->getConfig().getMaster().m_master.getHostName().c_str(), m_pApp->getConfig().getMaster().m_master.getPort(), errno));
    return -1;
  }
  CWX_UINT32 i = 0;
  ...
  if (0 != CwxMqConnector::connect(addr, unConnNum, fds, &timeouter, true)) {
    CWX_ERROR(
        ("Failure to connect to addr:%s, port:%u, err=%d", m_pApp->getConfig().getMaster().m_master.getHostName().c_str(), m_pApp->getConfig().getMaster().m_master.getPort(), errno));
    return -1;
  }
  m_uiCurHostId++;
  if (0 == m_uiCurHostId)
  ...
  ret = CwxMqPoco::packReportData(pTss->m_pWriter, pBlock, 0,
      m_pApp->getBinLogMgr()->getMaxSid(), false,
      m_pApp->getConfig().getCommon().m_uiChunkSize, NULL,
...
int CwxMqMasterHandler::recvMsg(CwxMsgBlock*& msg, list<CwxMsgBlock*>& msgs) {
  ...
  if (msg->length() < sizeof(ullSeq)) {
    CWX_ERROR(
        ("sync data's size[%u] is invalid, min-size=%u", msg->length(), sizeof(ullSeq)));
    return -1;
  }
...
int CwxMqMasterHandler::dealErrMsg(CwxMsgBlock*& msg, CwxMqTss* pTss) {
  ...
  if (CWX_MQ_ERR_SUCCESS
      != CwxMqPoco::parseSyncErr(pTss->m_pReader, msg, ret, szErrMsg,
          pTss->m_szBuf2K)) {
    CWX_ERROR(
        ("Failure to parse err msg from master, err=%s", pTss->m_szBuf2K));
    return -1;
  }
