...
public:
  ///构造函数
  CwxMqQueueHandler(CwxMqApp* pApp, CwxAppChannel* channel) : CwxAppHandler4Channel(channel)
  {
    m_pApp = pApp;
    m_uiRecvHeadLen = 0;
    m_uiRecvDataLen = 0;
...
private:
  ...
  ///pack一个不包含mq消息体的消息包。
  CwxMsgBlock* packEmptyFetchMsg(CwxMqTss* pTss, int iRet, ///<状态码
    char const* szErrMsg ///<错误消息
    );
  ///发送消息，0：成功；-1：发送失败
  int replyFetchMq(CwxMqTss* pTss, CwxMsgBlock* msg,  ///<消息包
