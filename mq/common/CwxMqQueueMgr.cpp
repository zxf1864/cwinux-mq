...
CwxMqQueue::CwxMqQueue(string strName, string strUser, string strPasswd,
...
///2：达到了搜索点，但没有发现消息；
///-1：失败；
int CwxMqQueue::getNextBinlog(CwxMqTss* pTss, CwxMsgBlock*&msg, int& err_num,
    char* szErr2K) {
  int iRet = 0;
  msg = NULL;
  if (m_memMsgMap.size()) {
...
///2：达到了搜索点，但没有发现消息；
///-1：失败；
int CwxMqQueue::fetchNextBinlog(CwxMqTss* pTss, CwxMsgBlock*&msg, int& err_num,
    char* szErr2K) {
  do {
...
CwxMqQueueMgr::CwxMqQueueMgr(string const& strQueuePath, CWX_UINT32 uiFlushNum,
...
///-1：失败；
///-2：队列不存在
int CwxMqQueueMgr::getNextBinlog(CwxMqTss* pTss, string const& strQueue,
    CwxMsgBlock*&msg, int& err_num, char* szErr2K) {
  if (m_bValid) {
...
int CwxMqQueueMgr::endSendMsg(string const& strQueue, CWX_UINT64 ullSid,
    bool bSend, char* szErr2K) {
  if (m_bValid) {
    CwxReadLockGuard<CwxRwLock> lock(&m_lock);
    map<string, pair<CwxMqQueue*, CwxSidLogFile*> >::iterator iter =
        m_queues.find(strQueue);
...
///0：存在
///-1：其他错误
int CwxMqQueueMgr::addQueue(string const& strQueue, CWX_UINT64 ullSid,
    string const& strUser, string const& strPasswd, char* szErr2K) {
  if (m_bValid) {
